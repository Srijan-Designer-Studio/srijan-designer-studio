...
import { Plus, Trash2, Upload, Banknote } from "lucide-react";
  ...
  return (
    <div className="animate-in text-black fade-in slide-in-from-bottom-4 duration-500">
      <div className="mb-8 border-b border-gray-100 pb-5">
        ...
        <p className="text-[19px] text-gray-500 mt-1">Set the global price, online cash off, and upload custom add-ons for this product.</p>
      </div>

      {/* Base Pricing & Online Cash Off */}
      <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm mb-8">
        <h3 className="text-[13px] font-bold text-gray-800 mb-4 uppercase tracking-wide">Base Pricing & Discounts</h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="bg-green-50 border border-green-200 rounded-lg p-2 flex items-center justify-between">
            <div className="w-full">
              <label className="flex items-center gap-1.5 text-[12px] font-bold text-green-800 mb-1.5">
                <Banknote size={14} /> Online Payment Cash Off (₹)
              </label>
              <input
                type="number"
                placeholder="e.g. 50"
                value={formData.onlineCashOff || ""}
                onChange={(e) => updateFormData({ onlineCashOff: e.target.value })}
                className="w-full text-[13px] border border-green-300 rounded-md px-3 py-1.5 outline-none focus:border-green-500 bg-white"
              />
            </div>
          </div>
        </div>
