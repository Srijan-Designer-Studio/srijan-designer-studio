...
export default function EditBlog({ params }) {
  ...
  const [form, setForm] = useState({
    ...
    existing_image: "", author: "Admin", published_at: "", 
  });
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      if (blog) {
        ...
        setForm({
          title: blog.title || "", content: blog.content || "", metaTitle: blog.meta_title || "",
          metaDescription: blog.meta_description || "", keywords: blog.keywords || "",
          permalink: blog.slug || "", category: blog.category_id || "", existing_image: blog.image_url || "",
          author: blog.author || "Admin", published_at: pubDate,
        });
      }
      setFetching(false);
    };
        ...
        <form onSubmit={handleSubmit} className="space-y-6 text-black">
          ...
          <div className="space-y-3">
            ...
            {form.existing_image && !image && (
              <div className="relative w-40 h-24 rounded-lg overflow-hidden border border-gray-200"><Image src={form.existing_image} alt="Cover" fill className="object-cover" unoptimized /></div>
            )}
          ...
          <div className="border border-gray-300 rounded-lg overflow-hidden text-black">
            {ClassicEditor ? (
              <CKEditor editor={ClassicEditor} data={form.content} config={{ extraPlugins: [CustomUploadAdapterPlugin], heading: { options: [ { model: "paragraph", title: "Paragraph", class: "ck-heading_paragraph" }, { model: "heading1", view: "h1", title: "Heading 1", class: "ck-heading_heading1" }, { model: "heading2", view: "h2", title: "Heading 2", class: "ck-heading_heading2" } ] } }} onChange={(event, editor) => setForm({ ...form, content: editor.getData() })} />
            ) : <p className="p-4 text-gray-500">Loading Editor...</p>}
          </div>
          <div className="grid md:grid-cols-2 gap-4">
            <input placeholder="Meta Title" value={form.metaTitle} className="w-full border border-gray-300 rounded-lg p-3 focus:outline-none focus:ring-2 focus:ring-indigo-500" onChange={(e) => setForm({ ...form, metaTitle: e.target.value })} />
            <input placeholder="Keywords (comma separated)" value={form.keywords} className="w-full border border-gray-300 rounded-lg p-3 focus:outline-none focus:ring-2 focus:ring-indigo-500" onChange={(e) => setForm({ ...form, keywords: e.target.value })} />
          </div>
          <textarea placeholder="Meta Description" rows="3" value={form.metaDescription} className="w-full border border-gray-300 rounded-lg p-3 focus:outline-none focus:ring-2 focus:ring-indigo-500" onChange={(e) => setForm({ ...form, metaDescription: e.target.value })} />
