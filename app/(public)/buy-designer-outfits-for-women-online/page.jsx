...
export const metadata = {
  ...
  openGraph: {
    ...
    images: [{ url: '/images/logo3.jpg', width: 1200, height: 630, alt: 'Srijan Fashion Ethnic Wear Collection' }],
    locale: 'en_IN',
    type: 'website',
  },
};
...
export default async function EthnicWearPage() {
  ...
  // CRITICAL FIX: Changed from .eq() to .contains() for Array support
      ...
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            "@type": "BreadcrumbList", 
            "itemListElement": [{ "@type": "ListItem", "position": 1, "name": "Home", "item": "https://srijandesignerstudio.com" },
            { "@type": "ListItem", "position": 2, "name": "Ethnic Wear", "item": "https://www.srijandesignerstudio.com/ethnic-wear" }]
          })
        }}
      />
      <ScrollToTop />
      <div className="max-w-[1320px] mx-auto px-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 lg:gap-8">
          {products.length > 0 ? (
              ...
              return (
                <Link href={`/product/${product.slug || product.id}`} key={product.id} className="group flex flex-col items-center cursor-pointer">
                  <div className="relative w-full aspect-[3/4] rounded-[16px] border border-gray-300 overflow-hidden mb-4 bg-gray-50 transition-shadow duration-300 group-hover:shadow-xl">
                    {imageUrl ? (
                      <img src={imageUrl} alt={product.title} className="object-cover object-top transition-transform duration-700 group-hover:scale-105" sizes="(max-width: 768px) 50vw, 33vw" />
                    ) : (
            ...
            <div className="col-span-full text-center text-gray-500 py-10">No ethnic wear products found.</div>
          )}
        </div>
      </div>
    </main>
